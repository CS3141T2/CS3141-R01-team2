...
function commsByInterest($db, $username) {
    ...
    $data = $db->prepare("
        ...
        user as (
            ...
            from member )
        SELECT community.name, community.leader, count.count, if(ifnull(user.entry_no, 'No') = 'No', 'No', 'Yes') as joined
        from community  natural join count
                        left outer join user using(name)
        where name in (select user
                         from interests left join commInterests on interests.interest = commInterests.interest
                         where user=?) ");
